...
local YOU_DIED_TEXTURE = getTexture("media/ui/DarkSoulsDeath/you_died.png")
...
function ISPostDeathUI:render()
	...
	if self.waitOver and self.textY > -((FONT_HGT_LARGE + 2) * #self.lines) then
		...
		for _,line in ipairs(self.lines) do
			...
			getTextManager():DrawStringCentre(UIFont.Large, self.screenX + self.screenWidth / 2, y, line, 1.0, 0.0, 0.0, 1)
			y = y + FONT_HGT_LARGE + 2
		end
		if YOU_DIED_TEXTURE then
			self:drawTextureScaledAspect(YOU_DIED_TEXTURE, self.screenX + (self.screenWidth - self.screenWidth * 0.5) / 2,
				self.screenY + self.screenHeight * 0.06, self.screenWidth * 0.5, self.screenHeight * 0.3, 0.92, 1, 1, 1)
		end
