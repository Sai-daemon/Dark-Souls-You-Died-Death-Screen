...
function ISPostDeathUI:configButton(button)
	...
	local orig = button.onclick
	button.onclick = function(target, btn, ...)
		if DarkSoulsDeathAudioPlayClick then
			DarkSoulsDeathAudioPlayClick()
		end
		if orig then
			return orig(target, btn, ...)
		end
	end
end
...
function ISPostDeathUI:onRespawn()
	if MainScreen.instance:isReallyVisible() then return end
	if DarkSoulsDeathAudioRespawn then
		DarkSoulsDeathAudioRespawn()
	end
	setGameSpeed(1)
...
function ISPostDeathUI:removeFromUIManager()
	if DarkSoulsDeathAudioRestore then
		DarkSoulsDeathAudioRestore()
	end
